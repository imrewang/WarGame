...
public:
	// 攻击某格敌人的动画
	void startMove(QWidget* object, const int beginx, const int beginy, const int endx, const int endy, const bool isLast = true, const int duration = ANIMATION_DURATION);
signals:
	void attackAnimationFinished()const; // 动画开始执行
	void animationStarted()const; // 动画执行结束
	void nonFinalAnimationEnd()const;
};
