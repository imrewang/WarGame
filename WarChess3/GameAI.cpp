...
void GameAI::moveCharacter(const int id, Role* t_character[], const int characterNum)
{
    ...
    for (ull i = 0; i < m_Al.m_availableCell.size(); i++)
    {
        ...
        for (int k = 0; k < characterNum; k++) {
            if (m_character[k]->belong == m_nowCharacter->belong && k!=id && m_character[k]->pieceState != Role::DEAD) {
    ...
    if (m_minDist <= m_nowCharacter->attackRange&& m_nowCharacter->curMove>=0)
    {
        ...
        */
        ...
        if (m_nowCharacter->curMove > 3) {
            int attacksNumber = (m_nowCharacter->curMove-1) / 3 + 1;
            for (int i = 0; i < attacksNumber; i++) {
                QTimer::singleShot(i * (4 * ANIMATION_DURATION), this, [=]() {
                    m_nowCharacter->attackAnimation->startMove(m_nowCharacter, m_nowCharacter->cellX, m_nowCharacter->cellY,
                        m_character[m_attrackid]->cellX, m_character[m_attrackid]->cellY);
                    connect(m_nowCharacter->attackAnimation, &AttrackAnimation::animationFinished, this, [=] {
            ...
            m_nowCharacter->attackAnimation->startMove(m_nowCharacter, m_nowCharacter->cellX, m_nowCharacter->cellY,
                m_character[m_attrackid]->cellX, m_character[m_attrackid]->cellY);
            connect(m_nowCharacter->attackAnimation, &AttrackAnimation::animationFinished, this, &GameAI::waitFunc, Qt::UniqueConnection);
        }
